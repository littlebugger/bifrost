...
`AUTH` sent while a mail transaction is open — even with `listener.auth` configured — reuses the top-level table's `AUTH`-not-configured `502` row above, not this sub-table: it is neither advertised nor meaningful mid-transaction, and this way it costs no eleventh reply. RFC 4954 would permit `503` instead — `502` is a deliberate simplification; no credential bytes are ever relayed to the backend either way.
...
| D4 | **Fresh backend connection per transaction is the default; session-affine reuse is a documented opt-in** (`reuse_envelopes` pool knob) | Haraka removed pooling wholesale after years of bugs (#2788 key leaks, RSET-reuse races) — a cross-session pool with no per-connection freshness bound. The opt-in stays narrower than what burned Haraka: one reuse slot per *client session* (never shared across sessions), RSET-revalidated before every reuse (a dead cached conn is silently re-dialed, no health signal), and capped at `reuse_envelopes` envelopes so N doubles as the freshness guarantee. Full cross-session connection pooling remains post-v1 epic 12. Cost with the knob at its default (0/1, off): one TCP+EHLO(+TLS) per message + backend conn-rate pressure (documented; `max_transactions` caps it). |
...
- **Backend connection rate:** with `reuse_envelopes` at its default (0/1, off), fresh-per-transaction multiplies backend connect rate; backends with postscreen/anvil-style limits must whitelist the balancer. `max_transactions` caps concurrency per server. Session-affine reuse (`reuse_envelopes > 1`, see docs/operations.md) divides that rate by up to N per pool, at the cost of one cached idle connection per active client session.
- **Leastconn and reused connections:** a stashed (cached) backend connection is idle at a command boundary between envelopes — it holds no lease and is invisible to `leastconn`'s in-flight accounting, exactly like any other idle connection. `leastconn` balances on transactions currently attached, not on connections a session is holding in reserve for reuse.
