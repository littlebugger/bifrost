...
> Part of **smtp-balancer** — read `/PROJECT.md` first (it is the spec; this epic builds the repo it lives in).
...
- Module name `smtp-balancer` (binary-only repo; internal packages import as `smtp-balancer/internal/<pkg>`).
...
**Files:**
- Create: `go.mod`, `.gitignore`, `cmd/smtp-balancer/main.go`, directory stubs `internal/`, `docs/` (plans already exist), `examples/`, `test/integration/`, `test/chaos/`, `testdata/`

**Interfaces:**
- Produces: module `smtp-balancer`; `main.go` flags: `-version` (print version+exit 0), `-f <path>` (config path, unused yet), `-c` (check mode, unused yet — exits 2 "not implemented").

- [ ] **Step 1:** `git init`; write `.gitignore` (binaries, `/tmp`, coverage, `*.out`, fuzz corpus crashers stay tracked though: ignore only `testdata/fuzz/**/pending`)
- [ ] **Step 2:** `go mod init smtp-balancer`; set `go 1.25`
- [ ] **Step 3:** write `cmd/smtp-balancer/main.go` using stdlib `flag`: `-version` prints `smtp-balancer <version>` where version is a `var version = "dev"` ldflags hook; `-c`/`-f` parsed but `-c` exits 2 with "config check not implemented yet"
- [ ] **Step 4:** `go build ./...` and `go run ./cmd/smtp-balancer -version` both succeed
- [ ] **Step 5:** commit `feat: repo skeleton, go module, entrypoint`
...
- [ ] **Step 1:** write `.golangci.yml` enabling exactly: `govet, errcheck, staticcheck, revive, gofumpt, unused` (short list; no `depguard` yet)
...
- [ ] **Step 3:** `make lint test-race build` all exit 0
- [ ] **Step 4:** commit `feat: makefile targets + lint config`
...
- [ ] **Step 1:** implement in `Makefile` (note: `$$` in a Make recipe reaches the shell as `$` — do NOT quadruple them; a `$$$$` would become the shell PID):
  ```make
  verify-new:
  ...
  ```
- [ ] **Step 2:** write `scripts/verify_new_selftest.sh`: creates a temp package with one passing test `TestPresent` and one integration-tagged passing test `TestTagged`; asserts (a) `make verify-new PKG=<tmp> TESTS='TestPresent'` exits 0, (b) `make verify-new PKG=<tmp> TESTS='TestAbsent'` exits **non-zero** — the whole point, (c) `TESTS='TestTagged'` without TAGS exits non-zero, (d) with `TAGS=integration` exits 0
- [ ] **Step 3:** run the selftest; both assertions hold
- [ ] **Step 4:** commit `feat: verify-new target with self-test`
...
- [ ] **Step 1:** `ci.yml`: job `gate` on push/PR — setup Go 1.26.x, install golangci-lint + gofumpt, run `make lint test-race integration`; job `nightly` on `schedule` (cron daily) — `make fuzz-long chaos` plus `go test -race -count=3 ./...` (full-load wiring lands with epic 11)
- [ ] **Step 2:** `README.md`: one-paragraph description (from PROJECT.md intro), status badge, "read PROJECT.md; plans in docs/plans/"
- [ ] **Step 3:** `make lint test-race` still green
- [ ] **Step 4:** commit `feat: CI workflow + README`
