...
> Part of **Bifrost** — read `/PROJECT.md` first; the Transparency Contract is THIS epic's spec. Depends on epics 00–04.
...
**Consumes:** `session.TxnHandler` slot (03), `backend.Dial/Conn` (04), `smtpwire` (03), `fakesmtp/smtpdrv` (01). Epic-03 facts that bind you (historical note: as built, deadline re-arming happens via unexported split read/write setters inside internal/proxy, clamped to Txn.Expiry — the exported Txn.SetDeadline mentioned pre-implementation was deleted as dead API); `Txn.W` is an unsynchronized `bufio.Writer` — the reply pump serializes all client writes through the session's single-writer discipline; the pipelining queue you replay contains ONLY MAIL + consecutive RCPTs + at most one DATA.
**Produces:**
```go
// internal/proxy
type Relay struct{}
...
`cmd/bifrost` wires `session → relay → balance/health`. Every message a client sends enters `HandleTransaction`; everything the client learns about its message exits through the pump. This epic is where R3 and R4 become true.
...
- [x] **Step 1:** failing tests: `TestFailoverToSecondCandidate` (first fake SetDown → dial fails → second fake gets the transaction; client sees ONLY the second's verdicts; DialCount proves order), `TestConnectRetriesPerCandidate` (one hanging fake as sole candidate → exactly 2 dial attempts observed via DialCount — the "×2 attempts" contract), `TestFailoverExhausted451` (both down → MAIL answered `451 4.4.1`; **session stays open**; next MAIL works when a fake returns), `TestFailoverQueueAnswers` (pipelined MAIL+RCPT+RCPT+DATA, all candidates down → each queued command answered `451 4.4.1` in order; NOTE the queue holds ONLY MAIL + consecutive RCPTs + at most one DATA per the RFC-2920 sync-point batching epic-03 implements — RSET/NOOP/QUIT are answered by the session main loop and never reach the relay), `TestNoFailoverAfterFirstRelayedByte` (first fake accepts MAIL then dies at RCPT → NO silent retry; RCPT gets 451 4.4.1 — the invariant test), `TestHealthSignalsEmitted` (DialFailure on down candidate, Success on completed txn)
- [x] **Step 2:** run: fail
- [x] **Step 3:** implement walk over candidates with per-attempt connect budget; invariant flag `relayedBytes bool` guards retry
- [x] **Step 4:** `make verify-new PKG=./internal/proxy TESTS='TestFailoverToSecondCandidate TestConnectRetriesPerCandidate TestFailoverExhausted451 TestFailoverQueueAnswers TestNoFailoverAfterFirstRelayedByte TestHealthSignalsEmitted' TAGS=integration
- [x] **Step 5:** commit `feat(relay): ordered silent failover with relayed-byte invariant`
...
- [x] **Step 1:** failing tests: `TestNever354Synthesized` (`backend-rejects-data-command`: fake OnDATA `451 4.3.0` → client sees it; state back to in-transaction, RSET works), `Test354AndVerdictVerbatim` (`backend-4xx-after-dot-relayed`: OnEOD `452 4.2.2` relayed exactly), `TestDotStuffingIntegrity` (bodies with `..`-stuffed lines, `.\r` mid-line, split writes at hostile boundaries → fake `AssertWireBody` byte-exact — R4 proof), `TestReadAheadDrainedAtData` (client pipelines body bytes immediately after DATA in same segment; bufio read-ahead reaches backend after relayed 354, nothing lost/reordered), `TestDetachAfterVerdict` (after final-dot verdict, backend gets QUIT (Recorder), next MAIL dials fresh — DialCount+1)
- [x] **Step 2:** run: fail
- [x] **Step 3:** implement: forward DATA, relay backend's 354-or-error; on 354 relayed → session goroutine pipes client→backend via 32 KB buffer + DataFramer; per-direction bytes-moving watchdog via deadlines re-armed per chunk; detach after verdict relayed
- [x] **Step 4:** `make verify-new PKG=./internal/proxy TESTS='TestNever354Synthesized Test354AndVerdictVerbatim TestDotStuffingIntegrity TestReadAheadDrainedAtData TestDetachAfterVerdict' TAGS=integration
- [x] **Step 5:** commit `feat(relay): data pipe with framer + watchdog + detach`
